            ...
            HStack {
                ...
                VStack(alignment: .leading, spacing: 6) {
                    Text(Auth.auth().currentUser?.displayName ?? "Username not found")
                        ...
                        .bold()
                        .font(.title3)
                    Text(Auth.auth().currentUser?.email ?? "Email not found")
                        .foregroundColor(secondaryColor)
                        .font(.caption)
                }
            }
...
struct menuLink: View {
    ...
    var body: some View {
        ...
        .onTapGesture {
            handleMenuAction(text: text)
        }
    }

    private func handleMenuAction(text: String) {
        if text == "Logout" {
            Task {
                do {
                    try await Authentication().logout()
                } catch let e {
                    print("Error logging out: \(e.localizedDescription)")
                }
            }
        } else {
